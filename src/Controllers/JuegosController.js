...
export const saveJuego = async(req,res) =>{
	try {
		...
		if (validacion == '') {
			...
			nombre:nombre,niveles:niveles,fecha:fecha,imagen:'/uploads/'+req.file.filename
		})
...
const validar = (nombre,niveles,fecha,img,sevalida) =>{
	...
	if (niveles === undefined || niveles.trim() === '' || isNaN(niveles)) {
		errors.push('El número de niveles NO debe de estar vacío y debe de ser númerico')
	}
